...
global C
%Setting variables for the positions of the four atoms
global Vx Vy 
global x y 
global Fx Fy 
global newAtomX newAtomY
global AtomSpacing
global Phi 
global nAtoms %number of atoms?
global time Mass0 Mass1 Pty0in Pty1in
%global Mass2 Pty2in Pty2in
global LJEpsilon LJSigma Phi0 AtomType
global MinX MaxX MinY MaxY PhiTot KETot
global nAtoms0 %number of silicon atoms
global nAtoms1 %number of argon atoms
%global nAtoms2 %number of Magnesium atoms
global T T0 T1 MarkerSize
%global T2
...
%Limits on how big atom spacing can get before they drift off?
MaxX = max(x) * 1.5;
MinX = min(x) * 1.5;
...
%Calculating forces on atoms of type 0 and 1?
...
%Initializing kinetic energy for 2 masses?
KE0 = mean(V2_0) * Mass0 * 0.5;
KE1 = mean(V2_1) * Mass1 * 0.5;
%KE2 = mean(

%Setting total kinectic energy
...
%setting initial previous positions of the atom
xp = x - dt * Vx;
xpp = x - 2 * dt * Vx;
yp = y - dt * Vy;
ypp = y - 2 * dt * Vy;
...
while t < TStop
    ...
    if Method == 'FD'
        ...
        %setting the new position of the atom
        x = xp + dx;
        y = yp + dy;
    ...
    %updating the previous positions of the atom
    xpp = xp;
    ypp = yp;
